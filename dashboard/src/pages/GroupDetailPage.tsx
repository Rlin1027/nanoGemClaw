...
function SkillsPanel({ groupFolder }: { groupFolder: string }) {
  ...
  const handleToggle = async (skillId: string, currentlyEnabled: boolean) => {
    try {
      ...
      refetch();
      showToast(
        `Skill ${!currentlyEnabled ? 'enabled' : 'disabled'} successfully`,
        'success',
      );
    } catch (err) {
...
export function GroupDetailPage({ groupFolder, onBack }: GroupDetailPageProps) {
  const { group, loading, error, refetch, updateSettings } =
    useGroupDetail(groupFolder);
      ...
      <StatsCards
        stats={[
          ...
          { label: 'Avg Response', value: avgResponseTime },
          {
            label: 'Total Tokens',
            value: (
              group.usage.total_prompt_tokens +
              group.usage.total_response_tokens
            ).toLocaleString(),
          },
      ...
      <div className="space-y-2">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
          ...
          <div className="p-4 bg-slate-900/50 rounded-lg border border-slate-800">
            ...
            <select
              value={group.geminiModel || 'gemini-3-flash-preview'}
              onChange={(e) =>
                handleSettingChange({ geminiModel: e.target.value })
              }
              disabled={saving}
