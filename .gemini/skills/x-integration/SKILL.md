---
name: x-integration
description: X (Twitter) integration for NanoGemClaw. Post tweets, like, reply, retweet, and quote. Use for setup, testing, or troubleshooting X functionality. Triggers on "setup x", "x integration", "twitter", "post tweet", "tweet".
---
...
> **Compatibility:** NanoGemClaw v1.0.0. Directory structure may change in future versions.
...
1. **NanoGemClaw is installed and running** - Telegram connected, service active
...
npx dotenv -e .env -- npx tsx .gemini/skills/x-integration/scripts/setup.ts
# Verify: data/x-auth.json should exist after successful login

# 2. Rebuild container to include skill
./container/build.sh
# Verify: Output shows "COPY .gemini/skills/x-integration/agent.ts"

# 3. Rebuild host and restart service
npm run build
launchctl kickstart -k gui/$(id -u)/com.nanogemclaw
# Verify: launchctl list | grep nanogemclaw shows PID and exit code 0
```
...
| `NANOGEMCLAW_ROOT` | `process.cwd()` | Project root directory |
...
| `logs/nanogemclaw.log` | Service logs (contains X operation logs) | Ignored |
...
To integrate this skill into NanoGemClaw, make the following modifications:
...
```typescript
import { handleXIpc } from '../.gemini/skills/x-integration/host.js';
```
...
```typescript
// @ts-ignore - Copied during Docker build from .gemini/skills/x-integration/
...
Change build context from `container/` to project root (required to access `.gemini/skills/`):
```bash
# Find:
...
First, update the build context paths (required to access `.gemini/skills/` from project root):
```dockerfile
# Find:
...
COPY .gemini/skills/x-integration/agent.ts ./src/skills/x-integration/
```

## Setup

All paths below are relative to project root (`NANOGEMCLAW_ROOT`).
...
**Verify success:**
```bash
launchctl list | grep nanogemclaw  # Should show PID and exit code 0 or -
```
...
```bash
echo '{"content":"Test tweet - please ignore"}' | npx dotenv -e .env -- npx tsx .gemini/skills/x-integration/scripts/post.ts
```

### Test Like

```bash
echo '{"tweetUrl":"https://x.com/user/status/123"}' | npx dotenv -e .env -- npx tsx .gemini/skills/x-integration/scripts/like.ts
```
...
echo '{"content":"Test"}' | npx tsx .gemini/skills/x-integration/scripts/post.ts
```
...
grep -i "x_post\|x_like\|x_reply\|handleXIpc" logs/nanogemclaw.log | tail -20

# Script errors
grep -i "error\|failed" logs/nanogemclaw.log | tail -20
```
...
container run nanogemclaw-agent ls -la /app/src/skills/
```
