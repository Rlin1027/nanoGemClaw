...
Adds Parallel AI MCP integration to NanoGemClaw for advanced web research capabilities.
...
- **Non-blocking Design** - Uses NanoGemClaw scheduler for result polling (no container blocking)

## Prerequisites

User must have:
1. Parallel AI API key from https://platform.parallel.ai
2. NanoGemClaw already set up and running
...
Replace with:
```typescript
const allowedVars = ['GEMINI_API_KEY', 'PARALLEL_API_KEY'];
```
...
### 5. Add Usage Instructions to GEMINI.md

Add Parallel AI usage instructions to `groups/main/GEMINI.md`:
...
Verify the build:
```bash
echo '{}' | container run -i --entrypoint /bin/echo nanogemclaw-agent:latest "Container OK"
```
...
- Check task runs: `tail -f logs/nanogemclaw.log | grep "scheduled task"`
...
3. Remove Web Research Tools section from groups/main/GEMINI.md
4. Rebuild: `./container/build.sh && npm run build`
5. Restart: `launchctl kickstart -k gui/$(id -u)/com.nanogemclaw`
