---
name: add-gmail
description: Add Gmail integration to NanoGemClaw. Can be configured as a tool (agent reads/sends emails when triggered from Telegram) or as a full channel (emails can trigger the agent, schedule tasks, and receive replies). Guides through GCP OAuth setup and implements the integration.
---

# Add Gmail Integration

This skill adds Gmail capabilities to NanoGemClaw. It can be configured in two modes:
...
> How do you want to use Gmail with NanoGemClaw?
>
> **Option 1: Tool Mode**
...
>    - If prompted for consent screen, choose "External", fill in app name (e.g., "NanoGemClaw"), your email, and save
>    - For Application type, select **Desktop app**
>    - Name it anything (e.g., "NanoGemClaw Gmail")
>    - Click **Create**
...
Add this mount block (after the `.gemini` mount is a good location):
...
Append to `groups/GEMINI.md` (the global memory file):
...
Also append the same section to `groups/main/GEMINI.md`.
...
> - Create a Gmail label (e.g., "NanoGemClaw")
...
export const EMAIL_CHANNEL: EmailChannelConfig = {
  ...
  triggerValue: 'NanoGemClaw',  // the label name, address pattern, or prefix
  contextMode: 'thread',
...
export async function checkForNewEmails(): Promise<EmailMessage[]> {
  ...
  // Option 1: Use Gemini CLI with just gmail MCP
...
Then find the Telegram connection handler and add `startEmailLoop()` call after the message loop starts:

```typescript
// In the connection handler, after message processing starts:
startEmailLoop();
```
...
```bash
sleep 3 && tail -20 logs/nanogemclaw.log | grep -i email
```
...
```bash
tail -f logs/nanogemclaw.log | grep -E "(email|Email)"
```
...
5. Remove Gmail sections from `groups/*/GEMINI.md`
